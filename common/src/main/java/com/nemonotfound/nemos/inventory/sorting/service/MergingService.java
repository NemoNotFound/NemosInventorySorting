...
public class MergingService {
    ...
    public void mergeAllItems(AbstractContainerMenu menu, List<SlotItem> sortedSlotItems, int containerId) {
        sortedSlotItems.stream()
                .filter(slotItem -> slotItem.itemStack().getMaxStackSize() > 1)
                .collect(groupingBy(slotItem -> new MergeKey(slotItem.itemStack().getItem(), slotItem.itemStack().getTag())))
                .forEach((key, slotItems) -> mergeItems(menu, slotItems, containerId));
    }
    ...
    private void mergeItems(AbstractContainerMenu menu, List<SlotItem> slotItems, int containerId) {
        ...
        while (leftSlotIndex < rightSlotIndex && remainingCycles-- > 0) {
            ...
            if (!isFullStack(leftItem)) {
                inventorySwapService.performStackMerge(
                        menu,
                        minecraft.gameMode,
                        containerId,
    ...
    private record MergeKey(net.minecraft.world.item.Item item, net.minecraft.nbt.CompoundTag tag) {
    }
